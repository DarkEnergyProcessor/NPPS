...
$sid = intval($_GET['secretbox_id'] ?? (-1));
...
elseif($sid == 0)
{
	header("Location: /webview.php/secretbox/view_id0");
	exit;
}

$secretbox_info = $DATABASE->execute_query("SELECT name, r_list, sr_list, ur_list, sr_chance, ur_chance, sr_guarantee, sr_new, ur_new FROM `secretbox_list` WHERE id = $sid");
