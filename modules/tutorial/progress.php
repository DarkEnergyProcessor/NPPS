...
$ts = $DATABASE->execute_query('SELECT tutorial_state FROM `users` WHERE user_id = ?', 'i', $USER_ID)[0][0];
...
if($DATABASE->execute_query('UPDATE `users` SET tutorial_state = ? WHERE user_id = ?', 'ii', $REQUEST_DATA['tutorial_state'], $USER_ID) === false)
{
