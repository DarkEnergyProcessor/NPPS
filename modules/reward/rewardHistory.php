...
$reward_stmt = "SELECT * FROM `$present_table` WHERE `collected` IS NOT NULL ORDER BY `collected` DESC LIMIT 20";
...
switch($reward_category)
{
	...
	{
		// Members
		$reward_stmt = "SELECT * FROM `$present_table` WHERE item_type = 1001 AND `collected` IS NOT NULL $reward_filter ORDER BY `collected` DESC LIMIT 20";
		break;
	}
	case 2:
	{
...
foreach($DATABASE->execute_query($reward_stmt) as $items)
{
	$reward_list_out[] = [
		...
		'opened_date' => to_datetime($items['collected']),
